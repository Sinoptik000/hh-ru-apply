...
const COVER_TEMPLATE_MAP = {
  operations: [path.join(ROOT, 'config', 'cover-letter-ops.txt')],
  sales: [path.join(ROOT, 'config', 'cover-letter-sales.txt')],
};

function resolveTemplateType(rawType) {
  return rawType === 'sales' ? 'sales' : 'operations';
}

function loadCoverLetterTemplateRaw(templateType = 'operations') {
  const type = resolveTemplateType(templateType);
  const candidates = COVER_TEMPLATE_MAP[type] || COVER_TEMPLATE_MAP.operations;
  ...
  throw new Error(`Шаблон cover-letter-${type}.txt не найден. Добавьте файл config/cover-letter-${type}.txt`);
}

function loadCoverLetterTemplateHint(templateType = 'operations') {
  const t = loadCoverLetterTemplateRaw(templateType);
...
function parseVariantsFromPlainText(text) {
  const src = String(text || '').replace(/\r/g, '').trim();
  if (!src) return [];

  // Common delimiters if model returned plain text instead of JSON.
  const byHr = src
    .split(/\n\s*---+\s*\n/g)
    .map((x) => x.trim())
    .filter(Boolean);
  if (byHr.length >= 2) return byHr.slice(0, 3);

  const byHeaders = src
    .split(/\n(?=(?:[#>*\-\s]*)?(?:вариант|variant)\s*\d+[:.)\-\s])/i)
    .map((x) => x.replace(/^(?:[#>*\-\s]*)?(?:вариант|variant)\s*\d+[:.)\-\s]*/i, '').trim())
    .filter(Boolean);
  if (byHeaders.length >= 2) return byHeaders.slice(0, 3);

  // Last resort: use full response as one variant.
  return [src];
}

function extractOpenRouterMessageContent(data) {
  const message = data?.choices?.[0]?.message;
  const content = message?.content;
  if (typeof content === 'string' && content.trim()) return content;

  // Some providers may return structured content chunks instead of plain string.
...
async function requestCoverLetterJson({ apiKey, models, userPrompt }) {
  const maxAttempts = Math.min(
    5,
    Math.max(1, Number(process.env.COVER_LETTER_RETRY_ATTEMPTS || 3))
  );
  ...
  throw lastError || new Error('OpenRouter: неизвестная ошибка генерации');
}
 ...
 */
export async function generateCoverLetterVariants(record, cvBundle, options = {}) {
  ...
  const templateType = resolveTemplateType(options.templateType || record?.templateType);
  const templateRaw = loadCoverLetterTemplateRaw(templateType);
  const templateHint = loadCoverLetterTemplateHint(templateType);
  ...
  const preferredModel = resolveOpenRouterModelForRequest();
  const modelCandidates = Array.from(
    new Set([preferredModel, process.env.COVER_LETTER_FALLBACK_MODEL, 'openrouter/free'].filter(Boolean))
  );
...
Верни СТРОГО один JSON без markdown и без текста до/после:
{
  ...
  const { data, content, usedModel } = await requestCoverLetterJson({
    apiKey,
    models: modelCandidates,
    userPrompt,
  });

  let rawVariantsInput;
  try {
    const parsed = extractJsonObject(content);
    rawVariantsInput = parsed.variants;
  } catch {
    rawVariantsInput = parseVariantsFromPlainText(content);
  }
  const rawVariants = normalizeVariants(rawVariantsInput);
  ...
  const providerModel = data?.model || usedModel || preferredModel;

  return { variants, providerModel, templateType };
}
