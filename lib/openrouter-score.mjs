...
import { scoreVacancyCascade, getEnabledProviders } from './ai-cascade.mjs';
...
export async function scoreVacancyWithOpenRouter(vacancy, cvBundle, prefs) {
  // Пробуем каскадный вызов (OpenRouter → Google → Groq)
  const providers = getEnabledProviders();
  if (providers.length > 0) {
    try {
      return await scoreVacancyCascade(vacancy, cvBundle, prefs);
    } catch (e) {
      console.warn(`[AI-Cascade] Все провайдеры недоступны: ${e.message}`);
    }
  }

  // Fallback на старый метод (только OpenRouter)
  return scoreVacancyLegacy(vacancy, cvBundle, prefs);
}

async function scoreVacancyLegacy(vacancy, cvBundle, prefs) {
