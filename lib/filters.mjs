...
/**
 * Определяет тип места работы: 'офис' | 'гибрид' | 'удаленка' | 'не указано'
 */
export function determineWorkplaceType(textBlob) {
  const t = textBlob.toLowerCase();

  const isRemote = includesAny(t, [
    'удалённ', 'удаленн', 'удалённая', 'удаленная',
    'remote', 'work from home', 'work from anywhere',
    'дистанционн', 'дистанционная',
  ]);

  const isHybrid = includesAny(t, [
    'гибрид', 'гибридн', 'гибридная',
    'hybrid', 'hybrid work',
    'офис несколько раз', '2 раза', '3 раза',
    'пару раз в офис', 'пару раз в неделю',
    'несколько дней в офис', 'несколько дней в неделю',
  ]);

  const isOffice = includesAny(t, [
    'работа в офисе', 'офисный формат',
    'работа в офисе обязательна', 'только офис',
    'office only', 'on-site only',
  ]);

  if (isRemote) return 'удаленка';
  if (isHybrid) return 'гибрид';
  if (isOffice) return 'офис';
  return 'не указано';
}
...
export function runHardFilters(parsed, prefs) {
  ...
  return {
    ...
    workplaceType: determineWorkplaceType(blob),
  };
}
