...
- URL - источник навигационного состояния (active tab + ключевые модалки/экраны).
- Переключение вкладок работает через `History API` без перезагрузки.
- `Back/Forward` восстанавливают состояние вкладок и открытых URL-модалок.
- Прямой вход на route/deep-link должен открывать тот же UI-контекст.
- Для неизвестных путей используется redirect на `/vacancies/manual`.
...
### Вкладка "Вручную": блок вариантов письма

- Контейнер `.manual-variants-row` использует горизонтальный layout (`display: flex`) для расположения выбора вариантов и кнопки генерации в одной строке.
- Блок `.manual-template-select-wrap` размещается слева от выбора варианта и содержит label + select шаблона письма.
- Select `#manual-template-type` использует те же токены темы (`--panel`, `--text`, `--border`, `--accent`) и поддерживает disabled/focus состояния.
- Кнопка `#btn-generate-letter` (класс `.btn-generate-letter`) визуально привязана к блоку `Вариант` и располагается справа от радио-кнопок.
- На мобильных (`max-width: 760px`) `.manual-variants-row` переключается в вертикальный layout для сохранения читаемости.
...
```
Используется в том числе для сценария быстрого добавления через кнопку "➕ Добавить", когда карточка появляется мгновенно во вкладке "Вручную" и затем дозаполняется фоном.
...
### Full-screen Vacancy Detail Modal

- Диалог: `.modal-dialog--vacancy-detail` (`width: min(94vw, 1260px)`, `max-height: 92vh`)
- Контент: 2-колоночная сетка `.vacancy-detail-grid` (левая колонка данные вакансии, правая колонка письмо)
- Левая колонка:
  - блок ключ-значение `.vacancy-detail-kv`
  - AI summary/risks/tags
  - full description в `.vacancy-detail-description` (scroll внутри блока)
- Правая колонка:
  - select шаблона + select варианта + textarea письма
  - action row `Сохранить / Утвердить / Отклонить / Сгенерировать заново`
- Footer:
  - статус письма и timestamp последнего сохранения/обновления
- Мобильная адаптация: при `max-width: 980px` сетка становится в 1 колонку.
